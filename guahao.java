...
public class guahao extends JDialog{
	...
	/**  * This method initializes jundo   *    * @return javax.swing.JButton   */ private JButton getJundo() {  if (jundo == null) {   jundo = new JButton();   jundo.setBounds(new Rectangle(371, 349, 78, 28));   jundo.setText("取消");   jundo.addActionListener(new java.awt.event.ActionListener() {    public void actionPerformed(java.awt.event.ActionEvent e) {     //清空所添加的内容     jpatientName.setText("");     jpatientTel.setText("");     jpatientAddress.setText("");     jpatientAge.setText("");    }   });  }  return jundo; } /**  * This method initializes jdno   *    * @return javax.swing.JTextField   */ private JTextField getJdno() {  if (jdno == null) {   jdno = new JTextField();   jdno.setBounds(new Rectangle(427, 267, 202, 25));   jdno.setText("请改变科室来初始化!");   jdno.setForeground(new Color(252, 4, 4));   jdno.setEditable(false);  }  return jdno; }   //医生同名问题没解决??????????????????? //同步设置医生的工作号 private void setDno(String department, String name){  String sql = "select 工作号 from 医务人员表 where 科室 = '"+department     +"' and 姓名 = '"+name+"'";  try{   linkDataBase link = new linkDataBase(main_Frame.getdataName(0),main_Frame.getdataPassword(0));   ResultSet rs = link.getResultSet(sql);   while(rs.next()){    jdno.setText(rs.getString(1));   }   rs.close();   link.getconnection().close();  }catch(SQLException ee){   System.out.println(ee.toString());  } }  //设置医生姓名 private void setDoctorName(String type,String department){  String dtype;  if(type.equals("普通号")){   dtype = "医生";  }else{   dtype = "医师";  }  String sql = "select 姓名 from 医务人员表 where 科室 = '"+department      +"' and 职称 = '"+dtype+"'";  try{   linkDataBase link = new linkDataBase(main_Frame.getdataName(0),main_Frame.getdataPassword(0));   ResultSet rs = link.getResultSet(sql);   while(rs.next()){    jdoctor.addItem(rs.getString(1));   }   rs.close();   link.getconnection().close();  }catch(SQLException ee){   System.out.println(ee.toString());  } } //设置挂号费用 private void setCost(){  String sql = "select 费用 from 挂号种类 where 名称 = '普通号'";  String sql2 = "select 费用 from 挂号种类 where 名称 = '专家号'";  try{   linkDataBase link = new linkDataBase(main_Frame.getdataName(0),main_Frame.getdataPassword(0));   ResultSet rs = link.getResultSet(sql);   if(rs.next()){    pcost = rs.getInt(1);    jcost.setText(String.valueOf(pcost));   }   ResultSet rs1 = link.getResultSet(sql2);   if(rs1.next()){    zcost = rs1.getInt(1);   }   rs.close();   link.getconnection().close();  }catch(SQLException ee){   System.out.println(ee.toString());  } } /**  * This method initializes jdepartment   *    * @return javax.swing.JComboBox   */ private JComboBox getJdepartment() {  if (jdepartment == null) {   jdepartment = new JComboBox();   jdepartment.setBounds(new Rectangle(416, 211, 89, 27));   jdepartment.addItemListener(new java.awt.event.ItemListener() {    public void itemStateChanged(java.awt.event.ItemEvent e) {     //随着科室名的变化更新医生姓名     jdoctor.removeAllItems();     setDoctorName((String)jtype.getSelectedItem(),(String)jdepartment.getSelectedItem());    }   });  }  return jdepartment; }  //设置科室 private void setDepartment(){  String sql = "select 科室名 from 科室信息表 where 科室名 like '%科'";  linkDataBase link = new linkDataBase(main_Frame.getdataName(0),main_Frame.getdataPassword(0));  try{   ResultSet rs = link.getResultSet(sql);   while(rs.next()){    jdepartment.addItem(rs.getString(1));   }   rs.close();   link.getconnection().close();  }catch(Exception e4){   System.out.println(e4.toString());  }  } /**  * This method initializes jButton   *    * @return javax.swing.JButton   */ private JButton getJButton() {  if (jButton == null) {   jButton = new JButton();   jButton.setBounds(new Rectangle(213, 350, 86, 27));   jButton.setText("复诊");   jButton.addActionListener(new java.awt.event.ActionListener() {    public void actionPerformed(java.awt.event.ActionEvent e) {     String sql = "update 病人信息表 set 医生号 = (?),挂号种类 = (?),是否就诊 = '否' where 病历号 = (?)";     String sql2 = "select * from 病人信息表 where 病历号 = '"+jpatientNo.getText().trim()+"'";     linkDataBase link = new linkDataBase(main_Frame.getdataName(0),main_Frame.getdataPassword(0));     ResultSet rs = link.getResultSet(sql2);     try{      if(rs.next()){       rs.close();       PreparedStatement pstmt = link.getconnection().prepareStatement(sql);       pstmt.setString(1, jdno.getText().trim());       pstmt.setString(2, (String)jtype.getSelectedItem());       pstmt.setString(3,jpatientNo.getText().trim());       pstmt.executeUpdate();       JOptionPane.showMessageDialog(null, "复诊成功！", "提示！", JOptionPane.INFORMATION_MESSAGE);       pstmt.close();       jpatientName.setText("");       jpatientAge.setText("");       jpatientAddress.setText("");       jpatientTel.setText("");       jpatientNo.setText(patientNo+String.valueOf(count));      }else{       JOptionPane.showMessageDialog(null, "请输入所须复诊的病历号后再进行复诊！", "提示！", JOptionPane.WARNING_MESSAGE);      }      link.getconnection().close();     }catch(SQLException ee){      JOptionPane.showMessageDialog(null, "请选择医生后再复诊！", "提示！", JOptionPane.WARNING_MESSAGE);      System.out.println(ee.toString());     }    }   });  }  return jButton; }  }
